 ...
 */
class UuidSpec extends FunSuite with Matchers with BeforeAndAfter with OneInstancePerTest {
  val UUID_COLUMN_NAME = "uuid_column"
  val TEST_DATA = UUID.randomUUID()

  test(s"$UUID_COLUMN_NAME insert, select and delete") {
    val client = ClientManager.getInstance.startup()
    val insertQuery = s"INSERT INTO test.test_table1 (id, ${UUID_COLUMN_NAME}) VALUES(?,?)"
    ...
    val insertResult = {
      val param = {
        ...
        list.put(ColumnTypes.UUID.builder.build(), TEST_DATA)
    ...
    val selectQuery = s"SELECT ${UUID_COLUMN_NAME} FROM test.test_table1 where id = ?"
    ...
    val selectResult = {
      ...
      data.name should be(UUID_COLUMN_NAME)
      data.value.isInstanceOf[UUID] should be(true)
      data.value.asInstanceOf[UUID] should be(TEST_DATA)
    }

    val deleteQuery = s"DELETE ${UUID_COLUMN_NAME} FROM test.test_table1 where id = ?"
