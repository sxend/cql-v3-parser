 ...
 */
class ListSpec extends FunSuite with Matchers with BeforeAndAfter with OneInstancePerTest {

  val LIST_COLUMN_NAME = "list_column"
  val TEST_DATA: util.List[String] = List("alpha", "日本語", "記号 => (๑˃̵ᴗ˂̵)و").asJava

  test(s"$LIST_COLUMN_NAME insert, select and delete") {
    val client = ClientManager.getInstance.startup()
    val insertQuery = s"INSERT INTO test.test_table1 (id, ${LIST_COLUMN_NAME}) VALUES(?,?)"
    ...
    val insertResult = {
      val param = {
        ...
        list.put(ColumnTypes.LIST.builder.build(ColumnTypes.VARCHAR.builder.build()), TEST_DATA)
    ...
    val selectQuery = s"SELECT ${LIST_COLUMN_NAME} FROM test.test_table1 where id = ?"
    ...
    val selectResult = {
      ...
      data.name should be(LIST_COLUMN_NAME)
      data.value.isInstanceOf[util.List[_]] should be(true)
      data.value.asInstanceOf[util.List[String]] should be(TEST_DATA)
    }
    val deleteQuery = s"DELETE ${LIST_COLUMN_NAME} FROM test.test_table1 where id = ?"
