...
class BigintSpec extends FunSuite with Matchers with BeforeAndAfter with OneInstancePerTest {
  ...
  test(s"$BIGINT_COLUMN_NAME column insert, select and delete") {
    ...
    val insertResult = {
      val param = {
        ...
        list.put(ColumnTypes.BIGINT.builder.build(), TEST_DATA)
    ...
    val selectQuery = s"SELECT ${BIGINT_COLUMN_NAME} FROM test.test_table1 where id = ?"
    ...
    val selectResult = {
      ...
      data.name should be(BIGINT_COLUMN_NAME)
      data.value.asInstanceOf[Long] should be(TEST_DATA)
    }

    val deleteQuery = s"DELETE ${BIGINT_COLUMN_NAME} FROM test.test_table1 where id = ?"
