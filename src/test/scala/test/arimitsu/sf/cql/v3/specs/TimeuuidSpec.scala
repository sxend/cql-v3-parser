...
class TimeuuidSpec extends FunSuite with Matchers with BeforeAndAfter with OneInstancePerTest {
  ...
  test(s"$TIMEUUID_COLUMN_NAME insert, select and delete") {
    val client = ClientManager.getInstance.startup()
    val insertQuery = s"INSERT INTO test.test_table1 (id, ${TIMEUUID_COLUMN_NAME}) VALUES(?,?)"
    ...
    val insertResult = {
      val param = {
        ...
        list.put(ColumnTypes.TIMEUUID.builder.build(), TEST_DATA)
    ...
    val selectQuery = s"SELECT ${TIMEUUID_COLUMN_NAME} FROM test.test_table1 where id = ?"
    ...
    val selectResult = {
      ...
      data.name should be(TIMEUUID_COLUMN_NAME)
      data.value.isInstanceOf[UUID] should be(true)
      data.value.asInstanceOf[UUID] should be(TEST_DATA)
    }

    val deleteQuery = s"DELETE ${TIMEUUID_COLUMN_NAME} FROM test.test_table1 where id = ?"
